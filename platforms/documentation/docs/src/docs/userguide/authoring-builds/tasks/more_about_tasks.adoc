...
* == outcomes | console
* == label the task  | after Gradle execution task /
    ** -- based on --
        *** are there actions / task must execute ?
        *** were the actions -- executed by -- Gradle ?
...
* `(no label)` or `EXECUTED`
    ...
    ** use cases
        *** if Task has actions -> Gradle executed the actions
        *** if Task has NO actions & some dependencies -> Gradle executed >= 1 of the dependencies
            **** Check <<organizing_tasks.adoc#sec:lifecycle_tasks,Lifecycle Tasks>>

* `UP-TO-DATE`
    ** == *Task's outputs and inputs did not change.*
        *** Check <<incremental_build.adoc#incremental_build,Incremental Build>>
    ** use cases
        *** if Task has actions -> task -- tells, NO outputs changes to -- Gradle
        *** if Task has NO actions & some dependencies -> ALL the dependencies are `UP-TO-DATE`, `SKIPPED` or `FROM-CACHE`
            **** Check <<organizing_tasks.adoc#sec:lifecycle_tasks,Lifecycle Tasks>>

* `FROM-CACHE`
    ** == *Task's outputs -- could be found from a -- previous execution.*
    ** -- based on -- build cache
        *** Check <<build_cache.adoc#build_cache,Build Cache>>

* `SKIPPED`
    ** == *Task did not execute its actions.*
    ** use cases
        *** Task -- has been explicitly excluded from the -- CL
            **** Check <<command_line_interface.adoc#sec:excluding_tasks_from_the_command_line,Excluding tasks from execution>>
        *** Task -- has an -- `onlyIf` predicate / return false
            **** Check <<controlling_task_execution.adoc#sec:using_a_predicate,Using a predicate>>

* `NO-SOURCE`
    ** == *Task did not need to execute its actions.*
    ** use cases
        *** Task has inputs and outputs, BUT <<incremental_build.adoc#skip-when-empty,no sources>> found
            **** _Example:_ inputs were NOT found
