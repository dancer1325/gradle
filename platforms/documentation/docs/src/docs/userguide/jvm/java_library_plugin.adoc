...
* -- expands the capabilities of the -- <<java_plugin.adoc#java_plugin,Java Plugin (`java`)>> / -- provide -- specific knowledge about Java libraries
    ** exposes an API
    ** based on tasks
        *** compile Java source code -- via -- `compileJava` task
        *** generates Javadoc -- via -- `javadoc` task
        *** compiled classes -- are packaged into -- a ".jar", -- via -- `jar` task

[[sec:java_library_usage]]
== Usage

* add | your build script
