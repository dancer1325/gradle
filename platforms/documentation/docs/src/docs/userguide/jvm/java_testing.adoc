...
* There are many different testing libraries & frameworks & types of test
    ** ALL need to be part of the build
* goal
    ** Gradle handles differing requirements between and within builds /
        *** significant coverage -- with the -- 2 most common testing frameworks
            **** https://junit.org/[JUnit]
            **** https://testng.org/[TestNG]
        *** different ways to control how the tests are run (<<#sec:test_execution,Test execution>>)
        *** how to select specific tests to run (<<#test_filtering,Test filtering>>)
        *** What test reports are generated & how to influence the process (<<#test_reporting,Test reporting>>)
        *** how to find tests to run (<<#sec:test_detection,Test detection>>)
        *** grouping tests together -- via -- major frameworks' mechanisms  (<<#test_grouping,Test grouping>>)
* new configuration DSL / model test execution phases -- via the -- incubating <<jvm_test_suite_plugin.adoc#jvm_test_suite_plugin,JVM Test Suite>> plugin

[[sec:java_testing_basics]]
== The basics

* 👁️ ALL JVM testing -- revolves around -- 1 task type: link:{groovyDslPath}/org.gradle.api.tasks.testing.Test.html[Test] 👁️
    ** requirements to operate it
        *** link:{groovyDslPath}/org.gradle.api.tasks.testing.Test.html#org.gradle.api.tasks.testing.Test:testClassesDirs[Test.getTestClassesDirs()]
            **** == where to find the compiled test classes
        *** link:{groovyDslPath}/org.gradle.api.tasks.testing.Test.html#org.gradle.api.tasks.testing.Test:classpath[Test.getClasspath()]
            **** == execution classpath / classes under test + test library to use
    ** runs a collection of test cases -- via -- ANY supported test library (JUnit, JUnit Platform or TestNG)
    ** collates the results
    ** those results can -- via an instance of the link:{groovyDslPath}/org.gradle.api.tasks.testing.TestReport.html[TestReport] task type, turn into --  a report
* if you're using a JVM language plugin (_Example:_ <<java_plugin.adoc#java_plugin,Java Plugin>>)
    ** -> automatically, you get
        *** dedicated `test` source set / unit tests
            **** -- automatically creates -- <<java_plugin.adoc#java_source_set_configurations,corresponding dependency configurations>>
                ***** _Example:_ `testImplementation` & `testRuntimeOnly`
        *** `test` task of type `Test` / runs those unit tests
            **** configured by the previous source set
                ***** with
                    ****** appropriate execution classpath
                    ****** directory / contain the compiled test classes
                ***** attaching it to the `check` <<organizing_tasks.adoc#sec:lifecycle_tasks,lifecycle task>>
    ** required steps to do
        *** configure compilation and runtime dependencies
        *** add ANY necessary configuration | `test` task

.JUnit Platform & maximum heap size for the tests' JVM is 1 GB
====
...
* link:{groovyDslPath}/org.gradle.api.tasks.testing.Test.html[Test] task has many
    ** generic configuration options
    ** framework-specific configuration
        *** link:{javadocPath}/org/gradle/api/tasks/testing/junit/JUnitOptions.html[JUnitOptions],
        *** link:{javadocPath}/org/gradle/api/tasks/testing/junitplatform/JUnitPlatformOptions.html[JUnitPlatformOptions]
        *** link:{javadocPath}/org/gradle/api/tasks/testing/testng/TestNGOptions.html[TestNGOptions]


* if you want to set up your own `Test` task / own set of test classes ->
    ** create your own
        *** source set
        *** `Test` task instance
    ** Check <<#sec:configuring_java_integration_tests,Configuring integration tests>>
