...
* requirements
    ** install Gradle runtime | your machine
        *** <<installation.adoc#installation,Installation>>
* 👀 `wrapper` task built-in / EVERY "build.gradle" 👀
    ** | group "Build Setup tasks"
    ** <<command_line_interface.adoc#sec:listing_tasks,listing the tasks>>
    ** ⭐️if you execute -> generates the necessary Wrapper files | project directory ⭐️
...
* recommendations
    ** track -- through -- version control
        *** ALTHOUGH, SOME organizations do NOT allow it
* Wrapper files
    ** + .jar file
        *** are small
* `gradle/wrapper/gradle-wrapper.properties`
    ** generated Wrapper properties file
    ** allows
        *** storing the information -- as --
            **** *server hosting* of the Gradle distribution
            **** *type of Gradle distribution*
                ***** by default, the `-bin` distribution -- contains --
                    ****** ONLY the runtime
                    ****** NO sample code & documentation
            **** *Gradle version*
                ***** execute the build
                ***** == Gradle version / used -- via -- `wrapper` task -- to generate the -- Wrapper files
            **** *timeout*
                ***** optionally, in ms
                ***** uses
                    ****** download the Gradle distribution
            **** *validation of the distribution* URL
                ***** optional
                ***** *boolean*
...
* if you upgrade the Gradle version -> benefit from
    ** NEW features
    ** NEW improvements

* Wrapper files MUST be tracked

* ways to upgrade the Gradle version
    ** manually changing Wrapper's `gradle-wrapper.properties` 's `distributionUrl` property
    ** run the `wrapper` task / provide the target Gradle version
        *** 👀recommended one 👀
            **** Reason: 🧠 any optimizations are applied | project 🧠
        *** see <<#sec:adding_wrapper,Adding the Gradle Wrapper>>
        *** if you run 1! ->
            **** update `gradle-wrapper.properties`
            **** NOT change `gradle-wrapper.jar` | wrapper
        *** if you run 2 -> ALL wrapper files are completely up-to-date
        *** if you run 3 -> download the Gradle distribution binaries & update the `gradlew` and `gradlew.bat` files
        *** _Example1:_ upgrades the Wrapper -- to the -- `latest` version
+
[listing,subs=+attributes]
----
...
----
        ...
        *** _Example2:_ upgrades the Wrapper -- to the -- specific version
+
[listing,subs=+attributes]
----
...
* `./gradlew --version`
    ** check gradle has the expected version
