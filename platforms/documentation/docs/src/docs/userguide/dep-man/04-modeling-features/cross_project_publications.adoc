...
* scenario
    ** multi-project builds / one project -- consumes the -- another project's artifacts
    ** simplest one: `A` -- depends on -- `B`
        *** _Example:_ | Java ecosystem, == `A` -- would depend on the  --`jar` / -- produced by -- `B`
        *** == `A` -- depend on a -- _variant of_ `B`
        *** requirements
            **** for compilation, API dependencies of `B` / -- provided by the -- `apiElements`
            **** for runtime, runtime dependencies of `B` / -- provided by the -- `runtimeElements`
* TODO:
