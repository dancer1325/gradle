...
* check <<#sec:repository-types,ALL allowed types of repositories>>

[[sec:declaring_public_repository]]
== Declaring a publicly-available repository

* public binary repositories
    ** allows, about open source dependencies
        *** download
        *** consume
* popular public repositories
    ** <<#sub:maven_central,Maven Central>>
        *** `mavenCentral()`
            **** == built-in shorthand notations for it
    ** <<#sub:maven_google,Google Android>>
        *** `google()`
            **** == built-in shorthand notations for it
...
* ways to declare
    ** shorthand notations
        *** -- available via the -- link:{groovyDslPath}/org.gradle.api.artifacts.dsl.RepositoryHandler.html[RepositoryHandler] API
    ** <<#sec:declaring_custom_repository,spell out the URL of the repository>>
